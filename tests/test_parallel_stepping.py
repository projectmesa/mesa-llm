...
async def test_parallel_vs_sequential_performance():
    ...
    for num_agents in agent_counts:
        ...
        assert parallel_time <= sequential_time, f"Parallel ({parallel_time:.3f}s) should be <= sequential ({sequential_time:.3f}s) for {num_agents} agents"
...
def test_step_agents_multithreaded_optimized():
    ...
    for agent in agents:
        ...
        assert total_steps == 1, f"Agent {agent.agent_id} should have 1 total step, got {total_steps}"
